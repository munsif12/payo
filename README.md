...
```bash
cd services/backend && npm test           # 154 tests (money invariants, pending actions, guardian/risk rules, auth, e2e)
cd services/ai && uv run pytest           # 325 tests (tools, agent w/ fake model, intent matrix, SSE, bilingual prompt)
cd apps/mobile && npm test                # 365 tests (money format, SSE parser, voice loop, card parity, renderers)
