...
- [x] jest test: `backendUrl()` returns platform-correct default and env override wins.
- [x] Implement store/api/auth slice; app boots to login when no token.
- [x] Commit `feat(mobile): rtk-query client, auth store, guarded routing`.
...
- [x] Implement components; jest snapshot-free smoke test for PinPad digit logic.
- [x] Commit `feat(mobile): shared ui kit + full ur/en strings`.
...
- [x] Login as `ammi@payo.demo`/`1234` works against seeded backend; bad PIN shows Urdu error.
- [x] Signup → OTP screen displays code → verify → lands in tabs with ₨10,000.
- [x] Commit `feat(mobile): auth flow — login, signup, mock otp`.
...
- [x] Tabs render on both platforms; balance reveals on tap.
- [x] Commit `feat(mobile): tab shell + balance header`.
...
- [x] Seeded history renders; filter by bills works; tap opens receipt.
- [x] Commit `feat(mobile): activity list with filters and receipt`.
...
- [x] Commit `feat(mobile): shared confirm→pin→execute flow`.
...
- [x] Send ₨1,500 Ammi→Bilal E2E on simulator: balance drops, Activity shows it.
- [x] Bank send shows resolved title + ₨25 fee line.
- [x] Commit `feat(mobile): send money flows`.
...
- [x] My QR renders; resolve of a pasted payload opens send flow (camera untestable in sim — note in QA).
- [x] Commit `feat(mobile): qr my-code and scan-to-pay`.
...
- [x] K-Electric `0400012345678` returns Ammi's ₨4,320 bill; pay completes; bill shows paid on re-lookup.
- [x] Commit `feat(mobile): bills and recharge flows`.
...
- [x] Commit `feat(mobile): money requests`.
...
- [x] عمرہ فنڈ shows ₨1,20,000 / goal progress; deposit ₨2,000 works without PIN.
- [x] Card reveals seeded PAN; freeze round-trips.
- [x] Statement for last month generates and PDF opens.
- [x] English toggle flips whole UI to LTR English.
- [x] Commit `feat(mobile): pockets, card, statements, profile+language`.

### Task 12: Phase gate — Argent QA on both platforms

- [x] Full manual-QA pass via Argent on iOS simulator AND Android emulator:
...
- [x] All three test suites green. Roadmap Phase 3 → done. Commit `feat(mobile): phase 3 complete`.
