...
- [x] **R1.1 Models.** `User`: `email` optional (drop unique), `pinHash` optional,
...
- [x] **R1.2 Tokens.** `signSession(user)` (payload `{sub,email?,scope:'session'}`, 30d) and
...
- [x] **R1.3 Auth routes.** `POST /auth/request-otp {phone}` (zod `+92\d{10}`): find-or-create
...
- [x] **R1.4 Profile + due bills.** `PATCH /me {name?, urduName?, language?}` → updated
...
- [x] **R1.5 Gate.** `npm test` green; e2e smoke updated to the new auth; `docs/plans/
...
- [x] **R2.1 Prompt.** Both system prompts gain: "The user may write or speak in English,
  ...
  reply-language rule.
- [x] **R2.2 Tool.** `list_due_bills` → `GET /bills/due`; returns text + `bill` card per due
...
- [x] **R2.3 Gate.** `uv run pytest` green; commit.
...
- [x] **R3.1 Deps + fonts.** Add `lucide-react-native`, `@expo-google-fonts/plus-jakarta-sans`,
  `expo-haptics`. Load Plus Jakarta 400–800 + Nastaliq in `_layout.tsx`.
- [x] **R3.2 Tokens.** `tokens.ts`: `light`/`dark` palettes (spec §2 verbatim), `type`, `radius`,
...
- [x] **R3.3 UI kit** (`src/ui/`): `Text` (variants money/h1/h2/hl/body/sub/foot/cap; Urdu
...
- [x] **R3.4 Motion primitives** (`src/motion/`): `useRise(delayMs)` (300ms, ease-out
...
- [x] **R3.5 i18n.** Default language `en`; add every new string from the artboards to
...
- [x] **R3.6 Gate.** `npx jest` + `tsc --noEmit` green; a `/dev/kit` route renders the kit
  (temporary, deleted in R7); Argent screenshot of it; commit.

## Phase R4 — Mobile auth + shell

- [x] **R4.1 Auth screens** `app/(auth)/phone.tsx | otp.tsx | create-pin.tsx | enter-pin.tsx`
...
- [x] **R5.3 Wallet** `app/(tabs)/wallet.tsx` per artboard (BalanceCard with Send/Request/QR,
  QuickActions, due-bill strip, recent activity ×3 + See all → `/activity`). Balance count-up
  on change.
- [x] **R5.4 Gate.** Argent: greeting + stagger visible; tap "Pay a bill" → bill card → Yes →
...
- [x] **R6.1** Pay hub, Send (recipient/amount), Confirm, PIN gate, Success — per artboards
  (success: check lands 400ms then text rises 300ms; haptic success).
- [x] **R6.2** Activity (chips, month summary card, grouped rows), Receipt, Bills (due card +
...
- [x] **R6.3 Gate.** Argent pass over every screen (both themes on Home/Wallet); commit.

## Phase R7 — Polish, QA, docs

- [x] **R7.1** Delete `/dev/kit`; remove dead components/i18n keys; `tsc`, jest, backend,
  AI suites green.
- [x] **R7.2** Full Argent acceptance run (spec §7) on iOS; Android if the emulator boots
  within 10 min; `docs/qa/revamp/QA-REPORT.md`.
- [x] **R7.3** Update README, DEMO-SCRIPT (new auth + AI-first home), roadmap status; commit.
