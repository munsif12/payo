...
| 🔒 `GET /me` | → `{ user: { id, name, urduName?, email?, phone, avatar?, language, pinSet, dateOfBirth?: 'YYYY-MM-DD', age?: number, preferences: { proactiveGreeting }, guardian?: { name, phone } }, account: { id, balancePaisa }, card: { id, last4, frozen } }` — the guardian summary is name+phone only; no ids leak to the payer. `age` is derived from `dateOfBirth`; both are absent until the user gives one. |
| 🔒 `PATCH /me` | `{ name?, urduName?, language?, dateOfBirth?: 'YYYY-MM-DD', preferences?: { proactiveGreeting? } }` → updated `user` (partial update). `dateOfBirth` is an ISO calendar date parsed as UTC midnight; under 18 (or not a real date) → 400 `VALIDATION`. |
...
| 🔒 `GET /institutions?q=` | → `{ items: [{ id, name, urduName, kind: 'wallet'\|'bank', code, popular, domain, logoUrl }] }` — ~35 entries (5 wallets: PAYO, Easypaisa, JazzCash, SadaPay, NayaPay; ~30 banks); `q` matches name/urduName/code; popular first, then alphabetical. `domain` is the institution's real-world domain (seeded); `logoUrl` is derived from it via `logoUrlFor` (`src/lib/logos.ts`, Google favicon service `https://www.google.com/s2/favicons?domain=<domain>&sz=128`), not stored. |
| 🔒 `POST /transfers/resolve` | `{ institutionId, identifier }` → `{ title, institution: {id,name,urduName,kind,domain,logoUrl}, identifier, linkedUserId? }`. PAYO + phone → real user (404 `RECIPIENT_NOT_FOUND`, 400 `SELF_TRANSFER`); other wallets → phone normalised to `+92…`, deterministic title; banks → IBAN or 10–16 digit account no, deterministic title; else 400 `INVALID_IDENTIFIER`. |
| 🔒 `GET /recipients?q=` / `POST /recipients` / `DELETE /recipients/:id` | create: `{ nickname, institutionId, identifier }` → resolves + stores `{ id, nickname, title, institution: {id,name,urduName,kind,domain,logoUrl}, identifier, linkedUserId?, lastUsedAt }`; `q` matches nickname/title/identifier; list is most-recently-used first; duplicate `(userId, institutionId, identifier)` → 409 `ALREADY_SAVED`. Replaces `Contact`/`/contacts`. |
| 🔒 `POST /transfers` | `{ to: { recipientId } \| { institutionId, identifier }, amountPaisa, note?, riskFlags?: ['pressure_language'], riskTarget?: { institutionId, identifier } }` → `PendingAction` (resolves again server-side; fee: banks 2500, wallets 0). Legacy `phone`/`bank`/`contact` shapes and `/banks*` are removed. **Guardian/scam evaluation — two independent gates.** **Approval** (age-independent): `needsApproval = guardian set && (amountPaisa ≥ ceilingPaisa \|\| (newRecipient && amountPaisa ≥ 2000000) \|\| an applied `pressure_language` flag)` → `approval: { required: true, guardianId, status: 'waiting' }`. A ₨1,000 send to a brand-new recipient needs no approval. **Check-in** (`riskFlags`, age-dependent): an applied `pressure_language` asks EVERY user; `new_recipient_large` is added when the recipient is new AND — for a SENIOR (`age ≥ 60`, `src/lib/risk.ts`) — `amountPaisa` ≥ 25 % of balance OR ≥ 2000000, and for everyone else `amountPaisa` ≥ `ceilingPaisa`. For a senior, a check-in answered `someoneAsked: false` suppresses `new_recipient_large` for that same `institutionId`+`identifier` for 24 h (`CheckInClearance`); a `true` answer suppresses nothing. `expiresAt` is 30 min when an approval or a risk flag applies, else the usual 2 min. **New recipient** = no `PendingAction` of a paying kind (`send_money*` or `request_settlement`) with `status: 'completed'` whose payload matches this `institutionId` + `identifier` — so a saved-but-never-paid recipient is still new. **`riskTarget`** names the recipient the AI's pressure detection was about: the `pressure_language` flag is applied only when the send's recipient matches it (phones compared in canonical `+92…` form). Absent `riskTarget` → applied unscoped, as before. |
...
| 🔒 `GET /billers` | → `{ items: [{ id, name, urduName, category, domain, logoUrl }] }` (categories: electricity, gas, internet, water, mobile) — `domain`/`logoUrl` as for institutions. |
| 🔒 `GET /bills/due` | → `{ items: [{ billId, biller: {id,name,urduName,category,domain,logoUrl}, consumerNo, amountPaisa, dueDate, month }] }` — the caller's own due bills (`Bill.userId`, stamped on lookup and by seed), refreshed to also include each saved biller's current due bill. |
...
| 🔒 `GET /saved-billers` / `POST /saved-billers` / `DELETE /saved-billers/:id` | create: `{ nickname, billerId, consumerNo }` → does the lookup and stores `{ id, nickname, biller: {id,name,urduName,category,domain,logoUrl}, consumerNo, consumerName }`; duplicate `(userId, billerId, consumerNo)` → 409 `ALREADY_SAVED`. |
| 🔒 `GET /telcos` | → `{ items: [{ id, name, urduName }] }` (Jazz, Zong, Telenor, Ufone) |
| 🔒 `POST /recharges` | `{ telcoId, phone, amountPaisa }` → `PendingAction` |
| 🔒 `GET /requests` / `POST /requests` | create: `{ fromPhone, amountPaisa, note? }`; incoming request approve: `POST /requests/:id/approve` → `PendingAction` (payer side); `POST /requests/:id/decline`. **Settling a request is an outgoing money path and runs the SAME guardian/scam evaluation as `POST /transfers`** (spec §A.6a), including the age-dependent check-in and the 24 h senior clearance: the recipient is the requester's PAYO identity (institution PAYO + their phone), which the payload also records — so a completed settlement and a completed transfer each make the other "not a new recipient". No client `riskFlags` are accepted on this path. |
...
**`Txn` shape:** `{ id, type: 'p2p'|'bank_transfer'|'bill'|'recharge'|'pocket_deposit'|'pocket_withdraw'|'request_settlement'|'card_unfreeze', direction: 'in'|'out', amountPaisa, feePaisa, counterparty: { name, urduName?, detail, institutionLogoUrl? }, category, status: 'completed', refNo, createdAt }` — `institutionLogoUrl` is set only for institution-based sends (`send_money`/`send_money_bank`/`send_money_wallet`), copied from the resolved institution's `logoUrl` at send time.
