...
import { RINGS, EASE_OUT } from './config';
...
function Ring({ size, color, delayMs, reducedMotion }: {
  ...
  useEffect(() => {
    ...
    progress.value = withDelay(
      delayMs,
      withRepeat(withTiming(1, { duration: RINGS.duration, easing: easeOut }), -1, false),
    );
  ...
  const style = useAnimatedStyle(() => {
    ...
    const scale = 1 + progress.value * (RINGS.scaleTo - 1);
    const opacity = RINGS.opacityFrom * (1 - progress.value);
...
// Two expanding, fading rings around the mic while the loop is listening.
// Spec §3 RINGS: 1.8s loop, ease-out, scale 1->2.1, opacity .5->0, second ring
// offset 600ms. Reduced motion: rings hidden entirely (no loop).
export function ListeningRings({ size = 64, color = '#F2A93B' }: { size?: number; color?: string }) {
  ...
  return (
    <View pointerEvents="none" style={{ width: size, height: size }}>
      ...
      <Ring size={size} color={color} delayMs={RINGS.offset} reducedMotion={reducedMotion} />
    </View>
  );
}
