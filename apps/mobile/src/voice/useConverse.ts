...
export function useConverse() {
  ...
  const playAudio = async (url: string) => {
    try {
      ...
      setTimeout(() => setStatus(cur => (cur === 'speaking' ? 'idle' : cur)), 4000);
    } catch {
      setStatus('idle');
    }
  };
  ...
  const run = (opts: { text?: string; audioUri?: string; userBubble: string | null }) => {
    if (status === 'thinking') return;  // speaking may be interrupted by a new turn
    soundRef.current?.remove();
    ...
    postSse(`${aiUrl()}/converse`, {
      ...
      onDone: () => setStatus(s => (s === 'thinking' ? 'idle' : s)),
      onError: () => {
